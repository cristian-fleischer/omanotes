...
// The slab behind a code block or a table bleeds out by the block padding
...
QPair<qreal, qreal> Backend::verticalMarginsFor(const QTextBlock &block, BlockKind kind) const {
    if (kind == BlockKind::Prose)
        return {0.0, 0.0};
    const auto needsRoom = [this, kind](const QTextBlock &neighbour) {
...
bool Backend::hasWantedTypography(const QTextBlock &block, BlockKind kind) const {
    ...
    const QPair<qreal, qreal> vertical = verticalMarginsFor(block, kind);
    return format.lineHeightType() == QTextBlockFormat::ProportionalHeight
        ...
        && qFuzzyCompare(format.topMargin() + 1.0, vertical.first + 1.0)
        && qFuzzyCompare(format.bottomMargin() + 1.0, vertical.second + 1.0);
}
...
void Backend::applyBlockTypography(QTextCursor &cursor, const QTextBlock &block, BlockKind kind) {
    ...
    blockFormat.setTopMargin(vertical.first);
    blockFormat.setBottomMargin(vertical.second);
...
void Backend::documentFontChanged() {
    if (m_highlighter)
        m_highlighter->refreshForDocumentFont();
}
...
void Backend::reapplyTypographyToChange() {
    ...
    // A block's vertical margins depend on the lines either side of it, so a
    // change to a line is a change to its neighbours' wants as well: emptying
    // the line above a fence is what lets the fence give its room back.
    const int touchedCount = touched.size();
    for (int i = 0; i < touchedCount; ++i) {
        if (touched.at(i).previous().isValid())
            touched.append(touched.at(i).previous());
        if (touched.at(i).next().isValid())
            touched.append(touched.at(i).next());
    }
